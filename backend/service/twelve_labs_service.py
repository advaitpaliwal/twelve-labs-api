...
class TwelveLabsService:
    ...
    def create_index(self, index_name : str):
        ...
        response = requests.post(self.INDEXES_URL, headers=self.HEADERS, json=data)
    ...
    def upload_video(self, index_id: str, file_name: str, file_path: str, language: str = 'en'):
        ...
        response = requests.post(self.TASKS_URL, headers=self.HEADERS, data=data, files=file_param)
        file_stream.close()
        return response.json()
    ...
    def generate_gist(self, video_id: str):
        ...
        response = requests.post(self.GIST_URL, headers=self.HEADERS, json=data)
        return response.json()
    ...
    def generate_summary(self, video_id: str, prompt: Optional[str] = None):
        ...
        if prompt:
            data['prompt'] = prompt
        response = requests.post(self.SUMMARIZE_URL, headers=self.HEADERS, json=data)
        return response.json()
    ...
    def generate(self, video_id: str, prompt: str):
        ...
        response = requests.post(self.GENERATE_URL, headers=self.HEADERS, json=data)
        return response.json()
